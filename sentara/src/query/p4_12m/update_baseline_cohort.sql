--  DROP IF EXISTS 
IF OBJECT_ID('SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m', 'U') IS NOT NULL
    DROP TABLE SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m;
...
semaglutide_raw AS (
    SELECT 
        ...
        CASE
            WHEN LOWER(de.drug_source_value) LIKE '%tablet%' OR 
                 LOWER(de.drug_source_value) LIKE '%oral%'  OR 
                LOWER(de.drug_source_value) LIKE '%rybelsus%' THEN 'Oral'
            ELSE 'Subcutaneous'
    ...
    SELECT 
        *,
		...
		CAST(DATEADD(MONTH, -24, 
				...
				) AS DATE) AS semaglutide_initiate_date_minus_24m,
		CAST(DATEADD(MONTH, -12, 
				MIN(start_date_comb) OVER (PARTITION BY person_id)
				) AS DATE) AS semaglutide_initiate_date_minus_12m,
		CAST(DATEADD(MONTH, 12, 
				MIN(start_date_comb) OVER (PARTITION BY person_id)
				) AS DATE) AS semaglutide_initiate_date_plus_12m,
		CAST(DATEADD(MONTH, 24, 
				MIN(start_date_comb) OVER (PARTITION BY person_id)
				) AS DATE) AS semaglutide_initiate_date_plus_24m,
        CAST(DATEADD(MONTH, 6, 
				MIN(start_date_comb) OVER (PARTITION BY person_id)
				) AS DATE) AS semaglutide_initiate_date_plus_6m,
        CAST(DATEADD(MONTH, -6,
                MIN(start_date_comb) OVER (PARTITION BY person_id)
                ) AS DATE) AS semaglutide_initiate_date_minus_6m,
...
anti_HTN_prescriptions_in_range AS (
    SELECT 
        ...
        de.drug_exposure_start_date,
        CASE 
            WHEN de.drug_exposure_start_date >= s.semaglutide_initiate_date_minus_6m AND de.drug_exposure_start_date < s.semaglutide_initiate_date THEN 1
            ELSE 0
        END AS pre_index,
        CASE 
            WHEN de.drug_exposure_start_date >= s.semaglutide_initiate_date AND  de.drug_exposure_start_date <= s.semaglutide_initiate_date_plus_6m THEN 1
            ELSE 0
        END AS post_index
    FROM ClinicalResearch.OMOP_Drug_Exposure de
    INNER JOIN ClinicalResearch.OMOP_Concept_Ancestor  ca
        ON ca.descendant_concept_id = de.drug_concept_id
    INNER JOIN semaglutide_study_period s
    ...
    WHERE ca.ancestor_concept_id IN (
        ...
        -- ndCCB
        1328165, 1307863
    ) AND de.drug_exposure_start_date BETWEEN s.semaglutide_initiate_date_minus_6m AND s.semaglutide_initiate_date_plus_6m
),
...
anti_HTN_ingredient_count AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN pre_index = 1 THEN ingredient_concept_id END) AS anti_HTN_pre_index_ingredient_count,
        COUNT(DISTINCT CASE WHEN post_index = 1 THEN ingredient_concept_id END) AS anti_HTN_post_index_ingredient_count
...
anti_hyperlipidemic_ingredient_count AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN pre_index = 1 THEN ingredient_concept_id END) AS anti_hyperlipidemic_pre_index_ingredient_count,
        COUNT(DISTINCT CASE WHEN post_index = 1 THEN ingredient_concept_id END) AS anti_hyperlipidemic_post_index_ingredient_count
...
concurrent_meds AS (
    ...
    FROM semaglutide_study_period s
...
SELECT * 
INTO SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m
