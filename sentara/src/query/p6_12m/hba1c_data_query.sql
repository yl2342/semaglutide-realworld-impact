WITH baseline_cohort AS (
    SELECT 
        *
    FROM 
        SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p6_12m
),
...
hba1c_combined AS (
    SELECT 
        ...
        CASE 
			...
			WHEN base.semaglutide_initiate_date_plus_24m <= hba1c.hba1c_visit_date 
                AND hba1c.hba1c_visit_date < base.semaglutide_initiate_date_plus_36m THEN '2'
			WHEN base.semaglutide_initiate_date_plus_36m <= hba1c.hba1c_visit_date 
                AND hba1c.hba1c_visit_date < base.semaglutide_initiate_date_plus_48m THEN '3'
            ELSE NULL 
        END AS period_12m
    FROM 
    ...
    WHERE
        hba1c.hba1c_visit_date BETWEEN semaglutide_initiate_date_minus_24m AND semaglutide_initiate_date_plus_48m        
),
