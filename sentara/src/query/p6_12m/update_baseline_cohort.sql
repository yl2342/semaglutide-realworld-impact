--  DROP IF EXISTS 
IF OBJECT_ID('SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p6_12m', 'U') IS NOT NULL
    DROP TABLE SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p6_12m;
...
semaglutide_study_period AS (
    SELECT DISTINCT
		...
		CAST(DATEADD(MONTH, 36, s.first_semaglutide_prescribe_date) AS DATE) AS semaglutide_initiate_date_plus_36m,
		CAST(DATEADD(MONTH, 48, s.first_semaglutide_prescribe_date) AS DATE) AS semaglutide_initiate_date_plus_48m
    FROM 
        semaglutide s
),
...
SELECT * 
INTO SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p6_12m
