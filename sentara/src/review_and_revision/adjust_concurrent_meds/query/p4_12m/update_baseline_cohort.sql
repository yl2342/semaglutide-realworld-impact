--  DROP IF EXISTS 
IF OBJECT_ID('SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m_rr_adjust_concurrent_meds', 'U') IS NOT NULL
    DROP TABLE SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m_rr_adjust_concurrent_meds;
...
semaglutide_study_period AS (
    SELECT DISTINCT
  	...
  	s.semaglutide_order_sum, 
		...
		CAST(DATEADD(MONTH, -24, s.first_semaglutide_prescribe_date) AS DATE) AS semaglutide_initiate_date_minus_24m,
...
-- First/last visit dates (2015 - 2025) for patients who initiated semaglutide
semaglutide_initiate_patients_first_last_visit_date AS (
    ...
    FROM (
        ...
        AND CAST(visit_start_datetime AS DATE) <= '2025-01-01'  -- Truncate to 2025-01-01
...
SELECT * 
INTO SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m_rr_adjust_concurrent_meds
