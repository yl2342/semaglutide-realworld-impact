--  DROP IF EXISTS 
IF OBJECT_ID('SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m_rr_two_prescriptions_6m_apart', 'U') IS NOT NULL
    DROP TABLE SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m_rr_two_prescriptions_6m_apart;
...
semaglutide_study_period AS (
    ...
    WHERE
        ...
        -- Ensure at least 6 months between first and last prescription
        AND DATEDIFF(MONTH, s.first_semaglutide_prescribe_date, s.last_semaglutide_prescribe_date) >= 6
),

-- First/last visit dates (2015 - 2025)
semaglutide_initiate_patients_first_last_visit_date AS (
    ...
    FROM (
        ...
        AND CAST(visit_start_datetime AS DATE) <= '2025-01-01'  -- Truncate to 2025
...
SELECT * 
INTO SandboxDClinicalResearch.YL_semaglutide_realword_impact_baseline_cohort_p4_12m_rr_two_prescriptions_6m_apart
