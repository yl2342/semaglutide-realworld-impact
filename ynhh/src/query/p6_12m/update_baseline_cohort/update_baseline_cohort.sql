...
WITH semaglutide_study_period_w_bmi AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN s.semaglutide_initiate_date_plus_24m <= b.bmiw_visit_date 
                AND b.bmiw_visit_date < s.semaglutide_initiate_date_plus_36m THEN '2'
            WHEN s.semaglutide_initiate_date_plus_36m <= b.bmiw_visit_date 
                AND b.bmiw_visit_date < s.semaglutide_initiate_date_plus_48m THEN '3'
            ELSE NULL 
        END AS period_12m
    FROM 
        PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/semaglutide_study_period.parquet') s 
    LEFT JOIN 
        PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/semaglutide_initiate_patients_first_last_visit_date.parquet') fl      
        ON s.person_id = fl.person_id
    LEFT JOIN 
        PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/bmiw_visit.parquet') b
        ON s.person_id = b.person_id
    WHERE 
        (s.semaglutide_initiate_date_plus_48m <= fl.last_visit_date)  -- Ensure in observation period
        AND (fl.first_visit_date <= s.semaglutide_initiate_date_minus_24m) -- Ensure in observation period
),

-- Select patients with BMI/weight data for all periods
selected_patients_w_all_periods AS (
    SELECT 
        person_id,
        COUNT(DISTINCT period_12m) AS period_12m_count,
        MAX(semaglutide_initiate_date_plus_48m) AS study_period_end,
...
baseline_cohort AS (
    SELECT 
        ...
        ss.semaglutide_initiate_date_plus_36m,
        ss.semaglutide_initiate_date_plus_48m,
    ...
    LEFT JOIN
        PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/semaglutide_study_period.parquet') ss
        ON sp.person_id = ss.person_id
    LEFT JOIN
        PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/selected_patients_w_t2dm.parquet') t
        ON sp.person_id = t.person_id
    LEFT JOIN
        PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/selected_patients.parquet') p 
...
TO '/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p6_12m/baseline_cohort.parquet' (FORMAT PARQUET);
