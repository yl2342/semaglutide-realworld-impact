...
bmiw_visit AS (
	SELECT 
		*
	FROM 
		PARQUET_SCAN('/home/jupyter/p2r2583347krumholz/Yuntian/semaglutide_realworld_impact/interim_tables/p8_6m/bmiw_visit.parquet')
),
...
weight_combined AS (
	SELECT 
		...
		CASE 
            ...
            WHEN base.semaglutide_initiate_date_plus_18m <= wt.bmiw_visit_date 
                AND wt.bmiw_visit_date < base.semaglutide_initiate_date_plus_24m THEN '3'
            WHEN base.semaglutide_initiate_date_plus_24m <= wt.bmiw_visit_date 
                AND wt.bmiw_visit_date < base.semaglutide_initiate_date_plus_30m THEN '4'
            WHEN base.semaglutide_initiate_date_plus_30m <= wt.bmiw_visit_date 
                AND wt.bmiw_visit_date < base.semaglutide_initiate_date_plus_36m THEN '5'
            ELSE NULL 
        END AS period_6m
	FROM 
...
-- designed to have weight data in all periods
weight_final AS (
  SELECT 
  	person_id,
  	period_6m,
