...
tryCatch({
    ...
    monthly_distinct_all_icd10 <- as.data.table(
        dbGetQuery(con,str_glue(paste(readLines(str_glue("query/{p_spec}/monthly_distinct_all_icd10.txt")), collapse = "\n")))
    )
    ...
    model2_random_effect(
      ...
      file.path(output_path, str_glue("ynhh_{p_spec}_{icd10_count}_m2.csv")),
      row.names = FALSE
    )
    ...
    model3_restrict_pretrend(
      ...
      file.path(output_path, str_glue("ynhh_{p_spec}_{icd10_count}_m3.csv")),
      row.names = FALSE
    )
    }
    ...
    monthly_all_visit <- as.data.table(
        dbGetQuery(con,str_glue(paste(readLines(str_glue("query/{p_spec}/monthly_all_visit.txt")), collapse = "\n")))
    )
    ...
    model2_random_effect(
      ...
      file.path(output_path, str_glue("ynhh_{p_spec}_all_{visit_count}_m2.csv")),
      row.names = FALSE
    )
    ...
    model3_restrict_pretrend(
      ...
      file.path(output_path, str_glue("ynhh_{p_spec}_all_{visit_count}_m3.csv")),
      row.names = FALSE
    )
    }
