...
option_list <- list(
  ...
  make_option(c("-r", "--exp-primary"),
              type="character",
              default=NULL,
              help="expenditure per icd10 code estimated from primary diagnosis [required]",
              metavar="PATH"),
...
tryCatch({
    ...
    # exp_all_processed <- as.data.table(read.csv(exp_all_path))
    ...
    # dbExecute(con, "INSTALL httpfs;") for first time
    ...
    monthly_distinct_primary_icd10 <- as.data.table(
        dbGetQuery(con,str_glue(paste(readLines(str_glue("query/{p_spec}/monthly_distinct_primary_icd10.txt")), collapse = "\n")))
    )
    ...
    log_message(sprintf("Monthly Expenditure (primary) among pooled cohort : mean %.2f ; sd %.2f", 
                      mean(exp_primary_cohort_pooled$total_expenditure),
                      sd(exp_primary_cohort_pooled$total_expenditure)))
    # diabetic
    exp_primary_cohort_diabetic <- exp_primary_cohort_pooled[had_t2dm_diag_before_initiation == 1]
    log_message(sprintf("Monthly Expenditure (primary) among diabetic cohort : mean %.2f ; sd %.2f", 
                      mean(exp_primary_cohort_diabetic$total_expenditure),
                      sd(exp_primary_cohort_diabetic$total_expenditure)))
    # non diabetic
    exp_primary_cohort_nondiabetic <- exp_primary_cohort_pooled[ had_t2dm_diag_before_initiation == 0]
    log_message(sprintf("Monthly Expenditure (primary) among non-diabetic cohort : mean %.2f ; sd %.2f", 
                      mean(exp_primary_cohort_nondiabetic$total_expenditure),
                      sd(exp_primary_cohort_nondiabetic$total_expenditure)))
