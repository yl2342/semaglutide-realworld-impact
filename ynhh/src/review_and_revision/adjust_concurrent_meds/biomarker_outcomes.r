...
option_list <- list(
  make_option(c("-o", "--output-dir"),
              type="character",
              default="results/estimates/biomarkers/p4_12m",
...
tryCatch({
    ...
    weight_data <- as.data.table(
        dbGetQuery(con,str_glue(paste(readLines(str_glue("query/{p_spec}/weight_data_query.sql")), collapse = "\n")))
    )
    ...
    # ) %>% write.csv(
    #     file.path(output_path, str_glue("ynhh_{p_spec}_weight_m1.csv")),
    #     row.names = FALSE
    # )
    ...
    model2_random_effect(
        ...
        label_list =c("Pooled","Diabetic","Non-Diabetic"),
        outcome = "weight_pct",
        covar_list = c("age_at_semaglutide_initiate","gender","race_ethn","has_concurrent_anti_HTN_meds","has_concurrent_anti_hyperlipidemic_meds"),
        min_patients = 10,
        model_label =  "2. Random individual effect"
    )%>% write.csv(
        file.path(output_path, str_glue("ynhh_{p_spec}_weight_m2.csv")),
        row.names = FALSE
    )
    ...
    model3_restrict_pretrend(
        ...
        file.path(output_path, str_glue("ynhh_{p_spec}_weight_m3.csv")),
        row.names = FALSE
    )
...
#     #  model 1: calendar effect only
#     log_message("Fitting model 1: calendar effect only model for blood_pressure data")
...
#             model_label =  "1. Calendar fixed effect only"
#         ) %>% write.csv(
#             file.path(output_path, str_glue("ynhh_{p_spec}_{bp_type}_m1.csv")),
#             row.names = FALSE
#         )
#     }
    ...
    # model 2: random effect
    log_message("Fitting model 2:  random effect model for blood_pressure data")
    ...
    for (bp_type in c("sbp","dbp")) {
        ...
        model2_random_effect(
            ...
            file.path(output_path, str_glue("ynhh_{p_spec}_{bp_type}_m2.csv")),
            row.names = FALSE
        )
    }
    ...
    # model 3: restrict pretrend
    log_message("Fitting model 3:  restrict pretrend model for blood_pressure data")
    ...
    for (bp_type in c("sbp","dbp")) {
        ...
        model3_restrict_pretrend(
            ...
            file.path(output_path, str_glue("ynhh_{p_spec}_{bp_type}_m3.csv")),
            row.names = FALSE
        )
    }
    ...
    #     model_label =  "1. Calendar fixed effect only"
    # ) %>% write.csv(
    #     file.path(output_path, str_glue("ynhh_{p_spec}_hba1c_m1.csv")),
    #     row.names = FALSE
    # )
    ...
    model2_random_effect(
        ...
        file.path(output_path, str_glue("ynhh_{p_spec}_hba1c_m2.csv")),
        row.names = FALSE
    )
    ...
    model3_restrict_pretrend(
        ...
        file.path(output_path, str_glue("ynhh_{p_spec}_hba1c_m3.csv")),
        row.names = FALSE
    )
    ...
    #     model_label =  "1. Calendar fixed effect only"
    # ) %>% write.csv(
    #     file.path(output_path, str_glue("ynhh_{p_spec}_total_cholesterol_m1.csv")),
    #     row.names = FALSE
    # )
    ...
    model2_random_effect(
        ...
        file.path(output_path, str_glue("ynhh_{p_spec}_total_cholesterol_m2.csv")),
        row.names = FALSE
    )
    ...
    model3_restrict_pretrend(
        ...
        file.path(output_path, str_glue("ynhh_{p_spec}_total_cholesterol_m3.csv")),
        row.names = FALSE
    )
