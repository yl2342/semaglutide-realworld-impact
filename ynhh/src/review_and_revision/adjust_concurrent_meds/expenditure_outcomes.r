...
tryCatch({
    ...
    monthly_distinct_primary_icd10 <- as.data.table(
        dbGetQuery(con,str_glue(paste(readLines(str_glue("query/{p_spec}/monthly_distinct_primary_icd10.sql")), collapse = "\n")))
    )
    ...
    for (exp in expenditure_outcomes) {
        ...
        # ) %>% write.csv(
        #   file.path(output_path, str_glue("ynhh_{p_spec}_primary_{exp}_m1.csv")),
        #   row.names = FALSE
        # )
        ...
        model2_random_effect(
          ...
          file.path(output_path, str_glue("ynhh_{p_spec}_primary_{exp}_m2.csv")),
          row.names = FALSE
        )
        ...
        model3_restrict_pretrend(
          ...
          file.path(output_path, str_glue("ynhh_{p_spec}_primary_{exp}_m3.csv")),
          row.names = FALSE
        )
    }
